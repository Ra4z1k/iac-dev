...
# Install PostgreSQL
if [ ! -f /usr/lib/postgresql/16/bin/postgres ]; then
  apt-get install --assume-yes --fix-broken --fix-missing postgresql-16
fi

# Configure PostgreSQL
if [ ! -f /etc/postgresql/16/main/.configured ]; then
  su postgres << 'EOF'
sed --in-place "s/#listen_addresses = 'localhost'/listen_addresses = '*'/" /etc/postgresql/16/main/postgresql.conf
...
cat << 'END' >> /etc/postgresql/16/main/pg_hba.conf
...
END
EOF
  touch /etc/postgresql/16/main/.configured
